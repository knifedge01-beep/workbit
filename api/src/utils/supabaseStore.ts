...
export async function readStoreSupabase(): Promise<Store> {
  ...
  const [projects, teams, members, views, roles, invitations, statusUpdates, statusUpdateComments, projectProperties, milestones, activity, issues, notifications] = await Promise.all([
    ...
    client.from('invitations').select('*').order('created_at', { ascending: false }),
  ...
  const projectPropertiesByTeam: Record<string, ReturnType<typeof rowToProjectProperties>> = {};
  ...
  return {
    ...
    invitations: (invitations.data ?? []).map((r) => rowToInvitation(r as DbRow)),
    statusUpdates: (statusUpdates.data ?? []).map((r) => rowToStatusUpdate(r as DbRow)),
    statusUpdateComments: (statusUpdateComments.data ?? []).map((r) => rowToStatusUpdateComment(r as DbRow)),
    ...
    notifications: (notifications.data ?? []).map((r) => rowToNotification(r as DbRow)),
  };
}
...
export async function writeStoreSupabase(store: Store): Promise<void> {
  ...
  const ordered = [
    ...
    { table: 'project_properties', rows: rows.project_properties, idColumn: 'team_id' as const },
    { table: 'status_updates', rows: rows.status_updates, idColumn: 'id' as const },
    { table: 'status_update_comments', rows: rows.status_update_comments, idColumn: 'id' as const },
    ...
    { table: 'notifications', rows: rows.notifications, idColumn: 'id' as const },
  ];
